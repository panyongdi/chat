            ...
            if (data.status == 200) {
                ...
                for (var i = 0; i < friendList.length; i++) {
                    friendListHTML +=
                        ...
                        '<div class="liRight">' +
                        '<span class="hidden-userId" style="display: none">' + friendList[i].id + '</span>' +
                        '<span class="hidden-linkId" style="display: none">' + friendList[i].linkId + '</span>' +
...
function getHistoryList() {
    $.ajax({
        ...
        async: true,
        success: function (data) {
            console.log("历史记录" + data.status);
            if (data.status == 200) {
                var history = data.data.historyList
                console.log(history)
                for (var i = 0; i < history.length; i++) {
                    var history = history[i]
                    processMsgBox.sendMsg(history.message, history.friendId, history.userId)
                }
            } else {
                console(data.msg);
            }
...
var ws = {
    ...
    singleSend: function (fromUserId, toUserId, content, linkId) {
        if (!window.WebSocket) {
            return;
        }
        if (socket.readyState == WebSocket.OPEN) {
...
$('.sendBtn').on('click', function () {
    ...
    if (news == '') {
        ...
        if (toUserId.length != 0) {
            ws.singleSend(fromUserId, toUserId, news, linkId);
        }
        ...
        // 消息框处理：
        processMsgBox.sendMsg(msg, toUserId, linkId)
...
$('.emjon li').on('click', function () {
    ...
    if (toUserId.length != 0) {
        ws.singleSend(fromUserId, toUserId, content, linkId);
    } else {
...
function friendLiClickEvent() {
    ...
    var linkId = $(this).children('.liRight').children('.hidden-linkId').text();
...
var processMsgBox = {
    sendMsg: function (msg, toUserId, userId) {
        // 1. 把内容添加到消息框
        $('.newsList').append(msg);
    ...
    sendFileMsg: function (msg, toUserId, toGroupId, userId) {
